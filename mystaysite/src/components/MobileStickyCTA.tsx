...
export default function MobileStickyCTA() {
  ...
  const whatsappUrl = `https://wa.me/${PHONE}?text=${encodeURIComponent(t.contact.whatsappPrefill)}`;

  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-[#0F172A] border-t border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.3)]">
      <div className="flex items-stretch">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-accent hover:bg-accent-dark text-white font-semibold text-sm transition-colors"
        >
          {t.mobileCta.sendLink}
        </button>
      </div>
    </div>
  );
